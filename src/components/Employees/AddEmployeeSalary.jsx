  ...
  return (
    <div className="fixed flex z-50 justify-center items-center py-4 top-0 left-0 bg-black/25 w-full h-full">
      <div className="flex flex-col p-4 bg-white w-[900px] h-auto rounded-md">
        ...
        <div className="px-4 flex flex-col gap-3" action="">
          <div className="flex flex-col  " >
            <label className="text-lg font-thin mb-2" htmlFor="salary">Salary</label>
            <input className="border border-gray-400 px-2 h-12 outline-none rounded-md" type="number" placeholder="e.g. 500.00"
            {...register("salary", {required: "Salary is required"})}
            />
            <span className="text-red-500">{errors?.salary && errors?.salary?.message}</span>
          </div>
          <div className="flex flex-col  ">
          ...
          />
          <span className="text-red-500">{errors?.date && errors?.date.message}</span>
          </div>
          <div className="flex gap-4 py-4">
            <button className="bg-slate-500 rounded-md w-36 px-4 py-2 text-lg text-white">Cancel</button>
            <button onClick={handleSubmit(submitForm)} className="bg-orange-500 rounded-md text-lg text-white px-4 py-2 w-36">Add</button>
          </div>
        </div>
      </div>
    </div>
